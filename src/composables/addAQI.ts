...
export function addQUI(url: string, options: UseKMLOptions = {}): AQILayer {
  ...
  const lastKnownVisible = ref<boolean>(options.visible || true); // Generated by Copilot
  ...
  const removeFromMap = (map: M.Map): void => {
    ...
    map.off('click', layerId, onEnterRef.value);
    map.off('mousemove', layerId, onMoveRef.value);
    map.off('mouseleave', layerId, onLeaveRef.value);
    if (popupRef.value) popupRef.value.remove();
    map.off('styledata', onStyleDataRef.value);
